...
class BaseController extends Controller
{
	...
	{
		$this->email->setFrom('deepak.dumraliya@potenzaglobalsolutions.com', 'Weekly Throw Down');
		$this->email->setTo($to);
	...
	public function get_pricepool_distribution($contest_id)
	{
		try {
			...
			#TIER 1
			$winners = round($numberofwinners / 3);
			...
			while ($n < $winners) {
				...
				if (($n > 0) && ($this->partcipants[$n]['SCORE'] == $this->partcipants[$n - 1]['SCORE'])) {
					...
					$samescoretier1 = $n;
					//}
					$wr[1][$n] = ["RANK" => $samescoretier1, "P_ID" => $this->partcipants[$n]['P_ID'], "SCORE" => $this->partcipants[$n]['SCORE']];
				} else {
			...
			$m = 0;
			$samescoretier2 = '';
			$winners =  round((2 * ($numberofwinners / 3)));
			while ($n < $winners) {

				if (($n > 0) && ($this->partcipants[$n]['SCORE'] == $this->partcipants[$n - 1]['SCORE'])) {

					$samescoretier2 = $n;
					$wr[2][$m] = ["RANK" => $samescoretier2, "P_ID" => $this->partcipants[$n]['P_ID'], "SCORE" => $this->partcipants[$n]['SCORE']];
				} else {
					if ($n >= round((2 * ($numberofwinners / 3)))) {
						break;
					}
			...
			$o = 0;
			$samescoretier3 = '';
			$winners = round((3 * ($numberofwinners / 3)));
			while ($n < $winners) {
				...
				if (($n > 0) && ($this->partcipants[$n]['SCORE'] == $this->partcipants[$n - 1]['SCORE'])) {
					...
					// }
					$wr[3][$o] = ["RANK" => $samescoretier3, "P_ID" => $this->partcipants[$n]['P_ID'], "SCORE" => $this->partcipants[$n]['SCORE']];
				} else {
					if ($n >= round((3 * ($numberofwinners / 3)))) {
			...
			$PPS_Adj = $PPS - ($ASI * ($WF - $W)) + ($ATI * ($W - $WT)); # 9 3
			$PPT_Adj = $PPT - ($ATI * ($W - $WT));
			...
			// print_r('NUMBER OF PARTICIPANT:-'.$numberofparticipants);
			// print_r("TOTAL PRICE POOL:- ".$PP );
			// print_r("<br>FIRST PRICE POOL:- ".$PPF );
			// print_r("<br>SECOND PRICE POOL:- ".$PPS );
			// print_r("<br>THIRD PRICE POOL:- ".$PPT );
			// print_r("<br>FIRST WINNERS COUNT:- ".$WF );
			// print_r("<br>SECOND WINNERS COUNT:- ".$WS );
			// print_r("<br>THIRD WINNERS COUNT:- ".$WT );
			// print_r("<br>FIRST PRICE POOL EACH:- ".$AF );
			// print_r("<br>SECOND PRICE POOL EACH:- ".$AS );
			// print_r("<br>THIRD PRICE POOL EACH:- ".$AT );
			// print_r("<br><b>AFTER ADJUSTMENT FIRST:- </b>".$AF_Adj);
			// print_r("<br><b>AFTER ADJUSTMENT SECOND:- </b>".$AS_Adj);
			// print_r("<br><b>AFTER ADJUSTMENT THIRD:- </b>".$AT_Adj);
			// print_r("<br><b>AFTER ADJUSTMENT TOTAL:-".$AJ_TOTOL."</b>");
	...
	{
		return round($numberofparticipants * 0.3);
	}
